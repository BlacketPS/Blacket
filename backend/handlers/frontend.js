...
export default (app) => {
    if (!["static", "proxy", "development"].includes(process.env.FRONTEND_TYPE)) return console.error(`Frontend configuration must be type of "static", "proxy" or "development".`) & process.exit(1);

    if (process.env.FRONTEND_TYPE === "static") {
    ...
    } else if (process.env.FRONTEND_TYPE === "development") {
